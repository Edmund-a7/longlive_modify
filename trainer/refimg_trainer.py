# Reference Image Flow Matching Trainer
# SPDX-License-Identifier: CC-BY-NC-SA-4.0
"""
参考图 Flow Matching 训练器

按照 Zero-to-Wan 项目风格实现，支持：
- FSDP 分布式训练
- LoRA 微调
- 梯度累积
- 自动检查点恢复
...
from utils.dataset import VideoReferenceImageDataset, collate_video_refimg, cycle
...
class RefImgFlowMatchingTrainer:
    """参考图 Flow Matching 训练器
    ...
    def __init__(self, config):
        ...
        if self.is_main_process:
            print(f"\n{'=' * 60}")
            print(f"Reference Image Flow Matching Training")
            print(f"{'=' * 60}")
        ...
        # ========================= Step 6: 数据加载器初始化 =========================
        if self.is_main_process:
            print("\n[6/6] Initializing dataloader...")

        dataset = VideoReferenceImageDataset(
            data_path=config.data_path,
            video_root=getattr(config, "video_root", None),
            image_root=getattr(config, "image_root", None),
            num_frames=config.num_training_frames,
        ...
        dataloader = torch.utils.data.DataLoader(
            ...
            pin_memory=True,
            collate_fn=collate_video_refimg
        )
    ...
    def _apply_lora(self):
        """应用新的 LoRA 配置到 generator（用于训练新增层时可选使用）"""
    ...
    def _find_latest_checkpoint(self, output_path):
        ...
        checkpoints = list(output_dir.glob("checkpoint_refimg_*/model.pt"))
        if not checkpoints:
            return None
    ...
    def save(self):
        ...
        if self.is_main_process:
            checkpoint_dir = os.path.join(
                self.output_path, f"checkpoint_refimg_{self.step:06d}"
            )
    ...
    def _cleanup_old_checkpoints(self, output_path, max_checkpoints):
        """清理旧检查点"""
        output_dir = Path(output_path)
        checkpoints = sorted(output_dir.glob("checkpoint_refimg_*"))
